...
namespace Assignment3.Utility
{
	public interface ILinkedListADT
	{
		...
		//reverses order of linked nodes
		void Reverse();

		//sort based on alphebetical order of user names in the list
		//leave node untouched if it isn't an user object
		void Sort();

		Object[] ToArray();
		ILinkedListADT JoinLists(params ILinkedListADT[] lists);
		ILinkedListADT[] Divide(int index);
	}
}
